...
- **`reservar.js`** — tres acciones en un mismo archivo (mismo patrón de `accion` que en `/api/gestion`), todas públicas (sin key). La creación (acción default) llama a `crearTurno()` de `lib/googleCalendar.js` — misma función que usa `api/chat.js`:
...
- **Radio en vivo** (`#radio-widget`, botón flotante abajo a la izquierda): 3 radios argentinas (Radio Mitre, La 100, Metro, streaming vía StreamTheWorld), play/pausa. El `<audio>` y el widget viven como **hermanos de `#app`** en el HTML (no como descendientes) — a propósito, para que el auto-refresh de la agenda cada 60s (que solo reemplaza `innerHTML` de elementos puntuales dentro de `#app`) nunca pueda cortar la reproducción. Se pausa al cerrar sesión.
