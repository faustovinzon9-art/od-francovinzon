...
async function obtenerLogoBuffer() {
  ...
  try {
    const resp = await fetch('https://od-francovinzon.vercel.app/assets/isologo.png');
...
// Logo arriba, texto abajo (en vez de lado a lado) — a propósito, para no depender de
// pdfkit posicionando texto junto a una imagen sin poder probarlo en este entorno (sin
// Node.js disponible). Todo el texto usa .text(str) sin coordenadas explícitas, mismo
// patrón ya probado en el resto de este archivo — pdfkit lo ubica en el margen
// izquierdo y avanza el cursor solo, sin riesgo de superposición.
async function dibujarEncabezadoMarca(doc, titulo, subtitulo) {
  const logo = await obtenerLogoBuffer();
  if (logo) {
    try {
      doc.image(logo, doc.page.margins.left, doc.y, { width: 32, height: 32 });
      doc.y += 40;
    } catch {
