...
export async function generarPdfReceta({ paciente, receta }) {
  ...
  // Espacio para firma y sello — la receta se imprime y se firma a mano.
  ...
  doc.y = lineaFirmaY + 30;

  doc.font('Helvetica').fontSize(8).fillColor(MUTED);
  if (receta.cuir) doc.text(`Receta electrónica original válida — Código/CUIR: ${receta.cuir}`);
