...
// QR/código de barras de la receta (ver el pedido, 2026-08-20: "la receta linda tiene
// que ser igual de funcional" que la original de MisRX/RCTA) — no se puede copiar el
// QR/barra real de la receta original (no hay forma de recortar una imagen de adentro
// de otro PDF sin sumar una librería pesada de renderizado de PDFs), así que se genera
// uno nuevo que codifica el mismo dato que hace funcional al original: el código/CUIR
// de recetario, que es lo que pide ingresar la validación on-line según el propio texto
// de la receta ("debe validarse on-line ingresando el número de recetario"). Nunca
// rompe la generación del PDF si falla (mismo criterio que el logo, ver
// obtenerLogoBuffer) — la receta sigue siendo válida sin el código gráfico, el CUIR en
// texto plano ya está más abajo igual.
...
export async function generarPdfReceta({ paciente, receta }) {
  ...
  // Espacio para firma y sello — la receta se imprime y se firma a mano. El QR y el
  // código de barras (mismo CUIR que la receta original) van a la derecha, a la misma
  // altura, para que la receta linda quede igual de funcional que la original: se puede
  // escanear tanto como escribir el número a mano en la validación on-line.
  const codigoQrY = doc.y;
  const [qrBuffer, barraBuffer] = await Promise.all([
    receta.cuir ? generarCodigoBuffer({ bcid: 'qrcode', text: receta.cuir, scale: 3 }) : null,
    receta.cuir ? generarCodigoBuffer({ bcid: 'code128', text: receta.cuir, scale: 2, height: 12 }) : null,
  ]);
  const colDerechaX = doc.page.width - doc.page.margins.right - 150;
  if (qrBuffer) {
    try { doc.image(qrBuffer, colDerechaX, codigoQrY, { width: 65, height: 65 }); } catch { /* imagen inválida, seguir sin QR */ }
  }
  if (barraBuffer) {
    try { doc.image(barraBuffer, colDerechaX + 75, codigoQrY + 12, { width: 75, height: 40 }); } catch { /* imagen inválida, seguir sin barra */ }
  }

  const lineaFirmaY = doc.y + 40;
  doc.moveTo(doc.page.margins.left, lineaFirmaY)
    .lineTo(doc.page.margins.left + 220, lineaFirmaY)
    .strokeColor(LINE).lineWidth(1).stroke();
  doc.font('Helvetica').fontSize(8).fillColor(MUTED)
    .text('Firma y sello', doc.page.margins.left, lineaFirmaY + 4);
  doc.y = Math.max(lineaFirmaY + 30, codigoQrY + 70);

  doc.font('Helvetica').fontSize(8).fillColor(MUTED).text('', doc.page.margins.left, doc.y);
  if (receta.cuir) doc.text(`Receta electrónica original válida — Código/CUIR: ${receta.cuir} (escaneable arriba)`);
