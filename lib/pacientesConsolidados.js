...
// RESTAURADA el 2026-08-24 (incidente): la sacó la limpieza de código muerto porque el
// backfill se había quitado de pacientes.js, pero después el backfill se restauró
// (decisión de mantenerlo hasta confirmar que corrió) y quedó un import roto — el
// módulo entero de pacientes devolvía FUNCTION_INVOCATION_FAILED en producción. Esta
// función sigue siendo SOLO para el backfill inicial (sobrescribe TODA la planilla de
// una sola vez con las filas ya armadas en memoria, en vez de upsertear una por una —
// evitaría un read-por-fila que con cientos de pacientes sería lentísimo, O(n²)). Se
// saca de nuevo junto con el backfill apenas se confirme que la planilla está poblada.
export async function reemplazarTodasLasFilas(filas) {
  const sheetId = await getOrCreateConsolidadosSheetId();
  const sheets = getPacientesSheetsClient();
  const ahora = new Date().toISOString();
  const values = filas.map((f) => [f.telefono, f.nombre || '', f.apellido || '', f.dni || '', f.fichaId || '', f.origen || 'turno', f.actualizado || ahora]);

  // Borra el rango de datos viejo (si la planilla tenía más filas antes que ahora, no
  // deja basura colgando al final) y escribe todo de nuevo.
  await conReintentos(() => sheets.spreadsheets.values.clear({ spreadsheetId: sheetId, range: RANGO_DATOS }));
  if (values.length) {
    await conReintentos(() => sheets.spreadsheets.values.update({
      spreadsheetId: sheetId,
      range: 'A2',
