...
// Versión batch de la pasada: por cada ficha UNA sola petición (values.batchGet) que
// trae identidad (C5:C7) y movimientos (B18:C2000) juntos — la cuota de lectura de
// Sheets es compartida con el consultorio en horario de atención, así que menos
// peticiones = menos riesgo de frenar a quien está atendiendo (lección 2026-08-25).
async function leerFichaBatch(sheets, fichaId) {
  const { data } = await conReintentos(() => sheets.spreadsheets.values.batchGet({
    spreadsheetId: fichaId,
    ranges: [`${SHEET_NAME}!C5:C7`, `${SHEET_NAME}!B18:C2000`],
  }));
  const porRango = data.valueRanges || [];
  const identidadRaw = (porRango[0]?.values || []).map((r) => (r[0] != null ? String(r[0]).trim() : ''));
  const identidad = {
    nombre: identidadRaw[0] || '',
    apellido: identidadRaw[1] || '',
    dni: identidadRaw[2] || '',
  };
  const fechas = new Set();
  (porRango[1]?.values || []).forEach((r) => {
    const tratamiento = r[1] != null ? String(r[1]).trim() : '';
    if (/^\[ANULADO\]/.test(tratamiento)) return;
    const iso = fechaMovimientoAISO(r[0]);
    if (iso) fechas.add(iso);
...
export async function pasadaRetroactivaConfirmacion({ dryRun = true, maxFichas = 0, offset = 0, desdeISO = '', hastaISO = '' } = {}) {
  try {
    ...
    for (const fila of slice) {
      // 1 sola petición por ficha (batchGet): identidad real + movimientos.
      let identidad;
      let fechas;
      try {
        ({ identidad, fechas } = await leerFichaBatch(sheets, fila.fichaId));
      } catch (err) {
        errores += 1;
        if (errores > 5) break; // cuota o falla sostenida: no seguir golpeando
        continue;
      }
