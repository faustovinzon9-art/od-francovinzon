...
// INCIDENTE 2026-08-12: envolverConReintentos() (lib/retry.js) tiraba producción entera
// abajo (500 en TODO endpoint que toca Calendar/Sheets/Drive, incluido el público
// api/disponibilidad.js) — el Proxy pasa `receiver` en Reflect.get(), y las clases de
// googleapis/google-auth-library (v174) usan campos privados nativos (#campo); un
// getter interno corriendo con `this` = el Proxy en vez de la instancia real tira
// "Cannot read private member from an object whose class did not declare it". Se
// revirtió el envolver-todo-el-cliente-con-Proxy a propósito, dejando los clientes
// sin envolver — NO reintentar de esta forma sin probarlo contra la API real primero
// (el mock/fetch local nunca lo hubiera detectado, googleapis no estaba disponible acá).
// lib/retry.js queda sin usar por ahora — ver decisions.md antes de reintentar esto.
