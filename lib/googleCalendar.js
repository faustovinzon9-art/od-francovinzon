...
// Los tres clientes salen envueltos con reintento automático (ver lib/retry.js) — así
// cualquier llamada (events.insert, events.list, spreadsheets.values.get, etc.) hecha
// desde cualquier endpoint reintenta sola ante una falla transitoria de Google, sin
// tener que tocar cada call site uno por uno.
export function getCalendarClient() {
  return envolverConReintentos(google.calendar({ version: 'v3', auth: getAuth() }));
}

export function getSheetsClient() {
  return envolverConReintentos(google.sheets({ version: 'v4', auth: getAuth() }));
}

export function getDriveClient() {
  return envolverConReintentos(google.drive({ version: 'v3', auth: getAuth() }));
}
