...
// Fila "fantasma": ninguna celda tiene un dato real, pero hay contenido de contaminación
// — específicamente el string 'FALSE'/'TRUE' que deja la validación de casilla aplicada
// por error a columnas de texto (ver celdaTextoLimpia en api/gestion/pacientes.js). Estas
// filas ensucian la hoja (la UI las filtra al mostrar, pero siguen ocupando filas y
// confundiendo la detección de fila libre). Un checkbox desmarcado (false) NO cuenta
// como fantasma: la plantilla tiene el checkbox de "Autorizado" en todo el rango y esas
// filas son normales. Usada por el utilitario de limpieza
// (modo=limpiar-filas-fantasma-una-vez en api/gestion/pacientes.js).
export function esFilaFantasma(fila) {
  const celdas = fila || [];
  const sinDatoReal = celdas.every((c) => !esCeldaConDatoReal(c));
  const tieneContaminacionString = celdas.some((c) => {
    if (typeof c !== 'string') return false;
    const s = c.trim();
    return s === 'FALSE' || s === 'TRUE';
  });
  return sinDatoReal && tieneContaminacionString;
}

// Normaliza una fecha de nacimiento leída de C8 a texto canónico "DD/MM/AAAA" (con pad).
// Sheets puede devolver el mes/día sin cero ("7/7/2026") cuando la celda quedó como
// fecha en vez de texto — el fix del 2026-08-24 evita que vuelva a pasar para datos
// nuevos; esta función sirve para normalizar las fichas existentes de una sola vez
// (modo=migrar-fecha-nacimiento-una-vez). Devuelve null si el valor no matchea
// D/M/AAAA con año de 4 dígitos (no parece fecha de nacimiento — no tocar).
export function normalizarFechaNacimientoTexto(valor) {
